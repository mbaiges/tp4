...
public class TerrainTreeSoundPlayer : MonoBehaviour
{
    ...
    void Start() {
        ...
        nextAt = timeBetweenMax; // We start with max time
        InitTrees();
        FindClosestTrees();
    }
    ...
    private float SpawnSoundOnTreeBase(AudioClip[] sources, List<GameObject> all, List<GameObject> nearestSpawned, int maxNearPlayer, float nextAt, float timeBetweenMin, float timeBetweenMax) {
        if (closestTrees.Count > 0 && all.Count < maxInMap && nearestSpawned.Count < maxNearPlayer && elapsedTime >= nextAt) {
            ...
            // Instantiate sound
            AudioClip clip = sounds[(int) Random.Range(0, sounds.Length-1)];
            GameObject soundSource = new GameObject(clip.name);
            ...
            src.clip = sounds[(int) Random.Range(0, sounds.Length-1)];
    ...
    private void HandleSounds() {
        float nextAt = SpawnSoundOnTreeBase(sounds, all, NearSoundDatas(all), maxNearPlayer, this.nextAt, timeBetweenMin, timeBetweenMax);
